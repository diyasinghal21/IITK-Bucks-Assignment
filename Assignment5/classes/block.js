...
module.exports = class Block {
    constructor(index = "", phash = "", bodyhash = "", target = "", time = "", nonce = "", blockhash = "") {
        this.index = index;
        this.phash = phash;
    ...
    async create_header() {
        ...
        const question4 = () => {
            return new Promise((resolve, reject) => {
                rl.question("Enter the path of the binary file of block data ", (answer) => {
    ...
    async create_data_hash() {
        ...
        const readfile = async () => {
            return new Promise((resolve, reject) => {
                const readStream = fs.createReadStream(this.path, { highWaterMark: 16 });
                const data = [];
    ...
    async create_block_hash() {
        ...
        list1.push(head_buff);
        list1.push(Buffer.concat(this.body_buff));
        var fbuff = Buffer.concat(list1);
        var hash = crypto.createHash("sha256");
        var hash_update = hash.update(fbuff, "binary");
        var generated_hash = hash_update.digest("hex");
